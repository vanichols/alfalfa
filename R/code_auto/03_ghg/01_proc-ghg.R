...
e_ghg <- 
  ...
  filter(time_horizon == a_gwp) #--assumed timeframe
...
e1 <- 
  read_csv("R/code_auto/02_energy/tulare_001-energy.csv") |> 
...
f_amount <- 
  ...
  filter(grepl(f_crop, assump_desc)) |> 
...
f4 <- 
  f3 |> 
  mutate(assump_id = f_id) |>#--they don't have a common column 
  left_join(f_dir)  |>  
  mutate(
...
f6 <- 
  f2 |> 
  bind_rows(f5) |> 
  fill(assump_id, .direction = "updown") |> 
...
f7_all <- 
  ...
  bind_rows(f_avoid1) |> 
  ungroup() |> 
  fill(production_id, .direction = "downup")
...
#---just look at it
# f8 |> 
#   mutate(co2_eq_kg = n2o_kg * a_gwpn2o) |> 
...
f12_vol <- 
  ...
  group_by(production_id, cat, unit, desc) |> 
...
f14_ind <- 
  f12_vol |> 
  bind_rows(f13_leach) |> 
  mutate(value = value * n_to_n2o * a_gwpn2o,
         unit = "kg co2e/stand") 
...
t3 <- 
  ...
  mutate(
    ...
    kgco2e_mg = value / (yield_kg_stand/1000)) |> 
...
t3 |> 
  write_csv(paste0("R/code_auto/03_ghg/", my_scenario, "-ghg.csv"))
