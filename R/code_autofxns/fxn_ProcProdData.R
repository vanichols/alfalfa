...
#--writes 'tulare_001-production.csv' within the 01_proc-prod folder

library(tidyverse)
library(readxl)


ProcProdData <- function(variant_id = "0002"){
  
  source("R/code_auto/00_conversions.R")
  source("R/code_auto/00_funs.R")
 
   
# get which a and p files to use ------------------------------------------

  var_log <- 
    read_csv("R/code_autofxns/man_variant_log.csv", skip = 5)   |> 
    filter(scenario == paste0("v_", variant_id))
  
  #--if it didn't change, use the 'base'
  var_change_file <- var_log |> pull(data_change)
  var_change_type <- var_change_file |> str_sub(1,1)
  
  p_file <- ifelse(var_change_type == "p", var_change_file, "p0001")
  a_file <- ifelse(var_change_type == "a", var_change_file, "a0001")
  
   # data --------------------------------------------------------------------
  
  d_raw <- read_csv(paste0("R/code_autofxns/datain/", p_file, ".csv"),
                    skip = 5) %>% 
