# example of how energy use is calculated for tulare base scenario
# created for phoo by gina 11/26
...
# get some info on assumptions -----------------------------------------------------

  d_o <- 
    read_csv("summary_for_phoo/tulare-scenario-notouch.csv")  |> 
    ProcDataIn()
  ...
  d_p <- read_csv("summary_for_phoo/tulare-scenario-notouch-production-data.csv")
  
  
  # 1. fert avoidance (a) --------------------------------------------
  
  #--what is the assumed following crop
...
#--this will now be a lookup table thing  
  ...
  s1 <- 
    read_csv("R/data_refs/ref_seed-energy-imperial.csv") |> 
      mutate(scenario_id = "scen_0000")
  ...
  e12 %>% 
    write_csv("summary_for_phoo/tulare-energy.csv")
